...
Dit is de Windows-variant van de Linux/Debian-versie
(ethernet_messenger.py) en is bedoeld als AANVULLING daarop voor gebruik
op de laptops van studenten — niet als vervanging van de Linux-versie.
De functionaliteit (Ethernet/ARP/IPv4/IPv6 opbouwen, visualiseren,
versturen en ontvangen) is identiek; alleen de platform-specifieke
onderdelen zijn aangepast:
  - interface-detectie (loopback/fysiek herkennen zonder
    /sys/class/net, dat op Windows niet bestaat)
  - rechten-check (Administrator i.p.v. root/sudo/setcap)
  - Wireshark-integratie (zoekt wireshark.exe op de gebruikelijke
    Windows-installatielocatie i.p.v. ervan uit te gaan dat het op
    PATH staat)

Vereist:
- Npcap (https://npcap.com/) — de Windows-tegenhanger van libpcap,
  nodig om via Scapy raw Ethernet frames te bouwen, versturen en te
  sniffen. Installeer met de standaardopties (WinPcap-compatibiliteit
  aangevinkt).
- Administrator-rechten om het programma te draaien (rechtermuisknop
  op het script/de opdrachtprompt → "Als administrator uitvoeren"),
  net zoals de Linux-versie root-rechten nodig heeft voor raw sockets.

Alleen bedoeld voor gecontroleerde lab-/testomgevingen (bijv. Cisco CML
of een geïsoleerd VLAN/hotspot tussen studentlaptops), niet voor
productienetwerken.
"""
...
try:
    ...
    print(
        "Scapy is niet geïnstalleerd. Installeer het (en PyQt6) met:\n"
        "  pip install PyQt6 scapy\n"
        "of gebruik een virtuele omgeving (zie README_windows.md).\n"
        "Zorg daarnaast dat Npcap geïnstalleerd is: https://npcap.com/"
    )
    sys.exit(1)
...
# Trefwoorden die duiden op een virtuele/niet-fysieke Windows-adapter
# (VPN's, virtualisatiesoftware, tunnels, ...). Windows heeft geen
# eenvoudig bestandssysteem-equivalent van Linux' /sys/class/net/.../
# device, dus dit is een heuristiek voor een redelijke standaardkeuze —
# geen garantie.
_VIRTUELE_ADAPTER_TREFWOORDEN = (
    "virtual", "vpn", "vmware", "virtualbox", "hyper-v", "hyperv",
    "tap-windows", "tap adapter", "loopback", "bluetooth", "teredo",
    "isatap", "wan miniport", "tunnel", "npcap loopback", "docker",
    "wsl", "pptp", "l2tp",
)


def _iface_beschrijving(iface_name):
    """
    Geeft de door Scapy/Windows gerapporteerde naam+beschrijving van
    een interface terug (lowercase, samengevoegd), of "" als dat niet
    lukt. Gebruikt bewust conf.ifaces.get() (directe dictionary-
    toegang) i.p.v. conf.ifaces.dev_from_name(): die laatste blijkt op
    Windows structureel te falen voor de \\Device\\NPF_{GUID}-paden die
    get_if_list() teruggeeft (een ValueError "Unknown network
    interface"), ook al staan ze wel degelijk met precies die naam in
    conf.ifaces.
    """
    iface_obj = conf.ifaces.get(iface_name)
    if iface_obj is None:
        return ""
    naam = getattr(iface_obj, "name", "") or ""
    beschrijving = getattr(iface_obj, "description", "") or ""
    return f"{naam} {beschrijving}".lower()


def is_loopback_interface(iface_name):
    """
    Bepaalt of een interface de (Npcap-)loopback-adapter is. Kijkt
    zowel naar de ruwe interfacenaam (Npcap noemt deze doorgaans
    letterlijk "\\Device\\NPF_Loopback") als naar de beschrijving, zodat
    dit ook werkt wanneer de beschrijving-lookup om wat voor reden dan
    ook niets oplevert.
    """
    if "loopback" in iface_name.lower():
        return True
    return "loopback" in _iface_beschrijving(iface_name)


def is_physical_interface(iface_name):
    """
    Bepaalt of een interface waarschijnlijk een fysieke netwerkkaart is
    (Ethernet- of wifi-adapter), op basis van trefwoorden in de
    Windows-adapterbeschrijving — in tegenstelling tot virtuele
    interfaces zoals VPN's, Hyper-V-switches of tunnel-adapters. Bij
    twijfel (onbekende beschrijving) wordt "fysiek" aangenomen, zodat
    een interface nooit onterecht wordt overgeslagen als standaardkeuze.
    """
    beschrijving = _iface_beschrijving(iface_name)
    if not beschrijving:
        return True
    return not any(trefwoord in beschrijving for trefwoord in _VIRTUELE_ADAPTER_TREFWOORDEN)


def is_windows_administrator():
    """Controleert of het programma met Administrator-rechten draait."""
    try:
...
def vind_wireshark_pad():
    """
    Zoekt het Wireshark-programma: eerst op PATH (voor het geval de
    installatie daaraan is toegevoegd), anders op de gebruikelijke
    Windows-installatielocaties. Geeft None terug als niets gevonden is.
    """
    op_path = shutil.which("wireshark") or shutil.which("Wireshark.exe")
    if op_path:
        return op_path
    mogelijke_paden = [
...
def is_geldig_ipv6(tekst):
    """IPv6-equivalent van is_geldig_ipv4() — zelfde reden (voorkomt dat
    Scapy's IPv6-laag bij een ongeldig/onvolledig adres een blokkerende
    DNS-lookup probeert)."""
    try:
        ipaddress.IPv6Address(tekst)
        return True
    except ValueError:
        return False


def effectief_ipv6(tekst):
    """
    Geeft tekst terug als het een geldig IPv6-adres is, anders "::" (de
    IPv6 unspecified address, het equivalent van "0.0.0.0") — een
    veilige, DNS-vrije placeholder voor live-preview berekeningen.
    """
    return tekst if is_geldig_ipv6(tekst) else "::"
...
def get_readable_interfaces():
    ...
    loopback-interface. Op Windows is scapy_interface_naam meestal een
    NPF-apparaatpad (\\Device\\NPF_{GUID}); de leesbare naam komt uit
    Scapy's eigen Windows-interfacebeschrijving (bijv. "Ethernet" of
    "Wi-Fi").
    """
    interfaces = []
    try:
        for iface_name in get_if_list():
            if is_loopback_interface(iface_name):
                continue
            readable = iface_name
            # conf.ifaces.get() i.p.v. dev_from_name() — zie de
            # toelichting bij _iface_beschrijving(). .name is op
            # Windows de korte, herkenbare verbindingsnaam (zoals in
            # "Netwerkverbindingen", bijv. "Ethernet" of "Wi-Fi");
            # .description is de uitgebreidere stuurprogrammanaam.
            iface_obj = conf.ifaces.get(iface_name)
            naam = getattr(iface_obj, "name", None) if iface_obj else None
            beschrijving = getattr(iface_obj, "description", None) if iface_obj else None
            if naam and beschrijving and naam != beschrijving:
                readable = f"{naam} — {beschrijving}"
            elif naam or beschrijving:
                readable = naam or beschrijving
...
class SnifferThread(threading.Thread):
    ...
    def __init__(self, iface, signals: SnifferSignals, ethertype=DEFAULT_ETHERTYPE):
        ...
        # Lokale raw-socket capture (Scapy/Npcap) vangt zowel inkomend
        # áls zelf verzonden verkeer op dezelfde interface af — dat
        # gebeurt onder de motorkap, niet doordat een switch het frame
...
class HoofdVenster(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ethernet Messenger — CCNA Lab Tool (Windows)")
        self.resize(760, 720)
        ...
        self._bouw_ui()
        self._controleer_beheerdersrechten()
    ...
    def _bouw_ui(self):
        ...
        # Waarschuwingslabel (Administrator-rechten)
    ...
    # ------------------------------------------------------------------
    # Beheerdersrechten-controle
    # ------------------------------------------------------------------
    def _controleer_beheerdersrechten(self):
        if not is_windows_administrator():
            self.root_waarschuwing.setText(
                "⚠ Dit programma draait niet als Administrator. Raw "
                "sockets (via Npcap) vereisen verhoogde rechten om "
                "Ethernet frames te versturen en te sniffen.\n\n"
                "Sluit het programma af en start het opnieuw met "
                "rechtermuisknop → 'Als administrator uitvoeren' (op het "
                "script, of op een Opdrachtprompt/PowerShell vanwaaruit "
                "je het script start).\n\n"
                "Zorg daarnaast dat Npcap geïnstalleerd is: "
                "https://npcap.com/ — zie README_windows.md."
            )
    ...
    def _vul_interfaces(self):
        ...
        # interface in de lijst (die op Windows vaak een VPN- of
        # virtualisatie-adapter is).
        standaard_index = 0
    ...
    def _interface_actie_toepassen(self, iface_name):
        ...
        if iface_name:
            ...
            try:
                ...
                # geen IPv4-adres heeft — dan laten we het veld leeg
                # zodat de student het zelf kan invullen.
    ...
    def _bouw_echt_ipv4_frame(self, dst_mac, src_mac):
        ...
        van het systeem, wat niet overeenkomt met wat de visualisatie
        toont ("0.0.0.0") en per lab-pc kan verschillen. Met deze
        expliciete fallback blijft de belofte "visualisatie =
        werkelijkheid" ook in dit geval gelden.
        """
    ...
    def _bouw_echt_ipv6_frame(self, dst_mac, src_mac):
        ...
        om dezelfde reden als bij IPv4.
        """
    ...
    def _bijwerken_protocol_status(self):
        """
        Toont/verbergt de ARP-, IPv4- en IPv6-subopties naar gelang het
        actieve EtherType, en dwingt bij ARP de bijbehorende Destination
        MAC en payload af in de gewone velden (die daarvoor tijdelijk
    ...
    def _bijwerken_visualisatie(self):
        ...
        if self._actief_ethertype == IPV4_ETHERTYPE:
            ...
            try:
                data_bytes = bytes(
                    IP(dst=effectief_ipv4(dst_ip), src=effectief_ipv4(src_ip), proto=CUSTOM_PROTOCOL_NUMMER)
            ...
            # afwijking tussen visualisatie en werkelijkheid. En net als
            # bij IPv4 voorkomt effectief_ipv6() dat een (nog) ongeldig/
            # onvolledig getypt adres Scapy's IPv6-laag een blokkerende
            # DNS-hostnaam-resolutie laat proberen.
            try:
                data_bytes = bytes(
                    IPv6(dst=effectief_ipv6(dst_ip), src=effectief_ipv6(src_ip), nh=CUSTOM_PROTOCOL_NUMMER)
                    / payload_tekst.encode("utf-8")
                )
            except Exception:
                data_bytes = b""
    ...
    def _verstuur_frame(self):
        ...
        if self._actief_ethertype == IPV6_ETHERTYPE:
            dst_ip6_tekst = self.ipv6_dst_ip_edit.text().strip()
            src_ip6_tekst = self.ipv6_src_ip_edit.text().strip()
            if not dst_ip6_tekst:
                QMessageBox.warning(self, "Fout", "Vul een Destination IPv6 in.")
                return
            if not is_geldig_ipv6(dst_ip6_tekst):
                QMessageBox.warning(self, "Fout", "Destination IPv6 is geen geldig IPv6-adres.")
                return
            if src_ip6_tekst and not is_geldig_ipv6(src_ip6_tekst):
                QMessageBox.warning(self, "Fout", "Source IPv6 is geen geldig IPv6-adres.")
                return
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Onvoldoende rechten",
                "Kan geen raw socket openen. Start dit programma als "
                "Administrator (rechtermuisknop → 'Als administrator "
                "uitvoeren') en zorg dat Npcap geïnstalleerd is "
                "(https://npcap.com/). Zie README_windows.md voor "
                "instructies.",
            )
        except Exception as e:
    ...
    def _start_sniffer(self):
        ...
        try:
            ...
            QMessageBox.critical(
                ...
                "programma als Administrator (rechtermuisknop → 'Als "
                "administrator uitvoeren') en zorg dat Npcap "
                "geïnstalleerd is. Zie README_windows.md voor "
                "instructies.",
            )
            self.sniffer_checkbox.setChecked(False)
    ...
    def _open_in_wireshark(self):
        ...
        wireshark_pad = vind_wireshark_pad()
        if not wireshark_pad:
            QMessageBox.critical(
                self,
                "Wireshark niet gevonden",
                "Wireshark kon niet gevonden worden op dit systeem. "
                "Installeer het via "
        ...
        try:
            subprocess.Popen([wireshark_pad, "-r", pcap_pad])
        except Exception as e:
...
def main():
    ...
    # showMaximized() direct aanroepen kan door sommige window managers
    # genegeerd worden als het venster nog niet geregistreerd is (dus
    # vóórdat de event-loop draait) — op Windows is dit meestal geen
    # probleem, maar de veilige QTimer-aanpak kost niets en is zo
    # consistent met de Linux-versie.
