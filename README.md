...
- **IPv6 opbouwen (Mode-D en hoger, EtherType IPv6 gekozen)**: werkt
  ...
  **Source IPv6** in plaats van de IPv4-velden (ook hier lokaal
  gevalideerd vóórdat Scapy ernaar kijkt). Source IPv6 wordt automatisch
  ingevuld met het globale (routeerbare) IPv6-adres van de gekozen
  interface, indien aanwezig — anders blijft het leeg. Ook hier blijven
  Destination MAC en Payload gewoon vrij invulbaar, toont de
